...
単に計算が煩わしい問題であると言えよう。例えば次の手法を用いるとよい。

+ 基準を用いた足し合わせ。すなわち系列$(m + a_1), dots, (m + a_n)$の和は
  $ m times n + (a_1 + dots.c + a_n) $
  と書けて、その平均は
  $ m + frac(a_1 + dots.c + a_n, n) $
+ 概数による比較。例えば$a <= alpha <= A$かつ$b <= beta <= B$であるとき、
  $ a / B <= alpha / B <= alpha / beta <= alpha / b <= A / b $

一つ一つの選択肢を確認していく。

- #quote[火曜日から日曜日までの会館日数]を$47$日を基準として計算すると、
  $ (plus.minus 0) + (+ 1) + (plus.minus 0) + (-7) + (+3) + (+2) = -1 $
  より、その平均は$47$日を上回らない。
- #quote[火曜日から日曜日までの一般書、児童書、雑誌の合計貸出数]を、千冊単位で切り上げて計算すると、次の表から$15$千冊との差の合計は$-6$千冊となり、これを上回らない。
  #figure(
    table(
      columns: (10em,) + (4em,) * 6,
      table.vline(x: 1),
      ...
      table.hline(),
      [合計貸出数(千冊)], $<=13$, $<=13$, $<=12$, $<=8$, $<=19$, $<=19$,
      [$15$千冊との差(千冊)], $<=-2$, $<=-2$, $<=-3$, $<=-7$, $<=4$, $<=4$,
      table.hline(),
    ),
  )
- #quote[合計貸出数に占める一般書貸出数の割合が$50%$を下回る曜日]を探すには、それぞれの曜日における一般書貸出数を$2$倍した値と合計貸出数を比較すればよい。
  #figure(
    table(
      columns: (12em,) + (4em,) * 6,
      table.vline(x: 1),
      ...
      [曜日], [火曜日], [水曜日], [木曜日], [金曜日], [土曜日], [日曜日],
      table.hline(),
      [合計貸出数(千冊)], $<=13$, $<=12.5$, $<=12$, $<=8$, $<=18.6$, $<=18.5$,
      [一般書貸出数$times 2$ (千冊)], $>=15$, $>=13$, $>=13$, $>=9$, $>=18.6$, $>=19$,
      table.hline(),
    ),
  )
  土曜日のみ詳細に計算すると、
  $ 8,288 + 960 = 9,248 < 9,315 $
  より、いずれの曜日も$50%$を下回らない。
- #quote[開館日$1$日当たりの貸出者数が最も少ない曜日は、木曜日である。]という記述について、例えば基準を$80$人とすれば、次より金曜日のほうが少ないことがわかる。
  $ 47 times 80 = 3760 < 3761 , wide 40 times 80 = 3200 > 2871 $
- #quote[貸出者$1$人当たりの貸出数が$3$冊を上回る曜日]を考えると、次の通り。
  #figure(
    table(
      columns: (12em,) + (4em,) * 6,
      table.vline(x: 1),
      ...
      [曜日], [火曜日], [水曜日], [木曜日], [金曜日], [土曜日], [日曜日],
      table.hline(),
      [合計貸出数(千冊)], $<=13$, $>=12$, $<=12$, $<=8$, $>=18$, $>=18$,
      [貸出者数$times 3$ (千冊)], $>=14$, $<=11.8$, $>=11$, $>=8.6$, $<=18.3$, $<=16.8$,
      table.hline(),
    ),
  )

  ここで木曜日について考えると
  $ 6,545 + 3,956 + 705 = 11,206 < 3,761 times 3 = 11283 $
  であり、土曜日について考えると
  $ 9,315 + 8,288 + 960 = 18,563 > 6,090 times 3 = 18,270 $
  となるから、水曜日、土曜日、日曜日は条件を満たす。
...
大きい桁の数どうしでの正確な商を求めるのはかなり面倒なので、商の比較を積を用いた比較になおしたい。ここでは次の二つを用いる。
1. ある整数$m$と$n$の比$display(frac(m, n))$がある数値$r$以上・以下であることを確かめたいとき
  $ m / n gt.eq.lt r <==> m gt.eq.lt n times r $
2. ある整数$m_1$と$n_1$の比$display(frac(m_1, n_1))$がある整数$m_2$と$n_2$の比$display(frac(m_2, n_2))$の$r$倍以上・以下であることを確かめたいとき
  $ m_1 / n_1 gt.eq.lt m_2 / n_2 times r <==> m_1 times n_2 gt.eq.lt m_2 times n_1 times r $

ただし、このままでは式の両辺がかえって膨大になってしまうため、たとえば分子分母を$1000$などで割ることで比較を簡単にできる。（注意。この手法は単に概数を用いるのと本質的には同等である。よって丸める方向に注意する必要がある。おおよその場合は整数部分を正確に計算すれば十分だろう。）

あるいは不等号の向きに注意して抑えてもよい。
一般的に除数が$10$の倍数であったほうが計算は容易であるから、例えば$n_1$や$n_2$に代わるキリの良い数を見つければよい。
ただしその際には不等号の向きに注意すること（示したいことがらにより代替となる数がもとの数より大きいべきか小さいかべきは変化する）。

- #quote[平成30年度の乗車人員$1$人当たりの走行キロ数は、法人タクシー、個人タクシーのいずれも$8 "km"$を超えている]かを、その値が$8 "km"$と仮定して計算し、比較する。
  #figure(
    table(
      columns: (12em,) + (8em,) * 2,
      table.vline(x: 1),
      ...
      [平成30], [法人タクシー], [個人タクシー],
      table.hline(),
      [総走行キロ数(千km)], $257,502$, $38,293$,
      [乗車人員$times 8$(千km)], $338,384$, $29,288$,
      table.hline(),
    ),
  )
  よって法人タクシーは実際には$8 "km"$を下回っているから、誤り。
- #quote[令和2年度の法人タクシーの総走行キロ数と乗車人員について、それぞれ前年度のそれらと比べると、いずれも前年度の$60%$以下である]かを、前年度の$60%$（A）と比較する。
  #figure(
    table(
      columns: (12em,) + (6em,) * 2,
      table.vline(x: 1),
      ...
      [法人タクシー], [令和2], [A],
      table.hline(),
      [総走行キロ数(千km)], $164,260$, $152,773.8$,
      [乗車人員(千人)], $25,440$, $23,892$,
      table.hline(),
    ),
  )
  よっていずれも前年度の$60%$を上回っているから、誤り。
- #quote[届出（認可）自動車$1$台あたりの輸送収入についてみると、令和3、4年度のいずれの年度においても、個人タクシーは法人タクシーの$50%$以下である。]という主張が正しいと仮定すれば、以下が成り立つ。
  $
    (3,454) / (1,202) attach(<=, t: ?) (34,982) / (4,413) times 0.50
    wide
    (4,951) / (1,147) attach(<=, t: ?) (42,343) / (4,495) times 0.50
  $
  分数部分の分子と分母を$1,000$で割り分母を払うと次の通り。よって成立する。
  $
    15. dots.c <= 21. dots.c wide 22. dots.c <= 24. dots.c
  $
- #quote[令和3年度の届出（認可）自動車$1$台当たりの走行キロ数についてみてみると、法人タクシーは個人タクシーの$2$倍を超えている。]という主張が正しいと仮定すれば
  $ (195,452) / (4,495) attach(>, t: ?) (33,980) / (1,147) times 2 $
  となり、先程と同様にして次が言えるので、誤り。
  $ 224. dots.c attach(>, t: crossmark) 305. dots.c $
- #quote[令和4年度の個人タクシーの乗車人員と輸送収入について、それぞれ前年度からの増加率を比べると、乗車人員のそれが大きい。]という主張が正しいと仮定すれば
  $ (3,004) / (2,339) attach(>, t: ?) (4,951) / (3,454) $
  となり、先程と同様にして次が言えるので、誤り。
  $ 10. dots.c attach(>, t: crossmark) 11. dots.c $
以上より、3.が正しい選択肢である。
#answer(3)

= #No(24) #Lv(2)

まずは、各工業における全国合計出荷額を算出する。

例えばある工業$A$の全国合計出荷額$S_A$を求める際には、工業$A$における京浜工業地帯の全国出荷額に占める割合$q_A$、京浜工業地帯における出荷額$P$とこれに占める工業$A$の出荷額の割合$p_A$を用いて
$ S_A approx P times frac(p_A, q_A) $
などとすればよい（数値は誤差を含むことに注意）。適当に丸めるとよいだろう。

この数値を用いて、各工業地帯・工業地域における各工業の出荷額を#[@syukkagaku]のように算出することができる。ただし実際には必要な部分だけ計算すればよい。

#figure(
  caption: [我が国のおもな工業地帯・工業地域における出荷額の構成],
  table(
    columns: (.7em,) + (5em,) + (5em,) * 5,
    table.vline(x: 2),
    table.hline(),
    table.cell(colspan: 2)[(百億円)], [金属工業], [機械工業], [化学工業], [食品工業], [その他],
    table.hline(),
    table.cell(rowspan: 3, text(size: .7em)[工業地帯]), [京浜], $195$, $1,088$, $521$, $281$, $205$,
    [阪神], $601$, $1,278$, $707$, $378$, $243$,
    [中京], $515$, $3,713$, $663$, $293$, $250$,
    table.hline(),
    table.cell(rowspan: 5, text(size: .7em)[工業地域]),
    [関東内陸], $339$, $1,210$, $595$, $491$, $248$,
    [京葉], $242$, $136$, $521$, $199$, $80$,
    [東海], $125$, $816$, $372$, $226$, $98$,
    [瀬戸内], $499$, $966$, $812$, $242$, $230$,
    [北九州], $144$, $394$, $118$, $152$, $80$,
    table.hline(),
    table.cell(colspan: 7, inset: 1pt, []),
    table.hline(),
    table.cell(colspan: 2)[全国合計], $3,900$, $13,600$, $6,200$, $3,900$, $2,500$,
    table.hline(),
  ),
) <syukkagaku>

この表を用いると、#quote[北九州工業地域において、機械工業の出荷額は、化学工業の出荷額を上回っている]ことがわかる。
また、逆にその他の選択肢は間違っていることが分かる。とくに次の表から選択肢2.は誤りと分かるだろう。

#figure(
  caption: [我が国のおもな工業地帯・工業地域における全国出荷額に占める割合],
  table(
    columns: (5em,) + (5em,) * 5,
    table.vline(x: 1),
    table.hline(),
    [], [金属工業], [機械工業], [化学工業], [食品工業], [その他],
    table.hline(),
    [工業地帯], $33.6%$, $44.7%$, $30.5%$, $24.4%$, $27.9%$,
    [工業地域], $34.6%$, $25.9%$, $39.0%$, $33.6%$, $29.4%$,
    table.hline(),
    table.cell(colspan: 6, inset: 1pt, []),
    table.hline(),
    [小計], $68.2%$, $70.6%$, $69.5%$, $58%$, $57.3%$,
    table.hline(),
  ),
)
...
$2$進法においてある有理数が*有限小数*で表されるための必要十分条件を問う問題である。

$2$進法において有理数$q$が有限小数で表されるのは、ある整数$m, n$が存在して
$ q = frac(m, 2^n) $
と表せるとき、かつそのときに限る。これは、$2$進法で小数第$n$位までの数は、これに$2^n$をかけることで整数となるからである。

例えば$0.25$は
$ 0.25 = 25/100 = 1/2^2 $
と表されるため$0.25 = 0.01_((2))$と表される。一方で$0.05$は
$ 0.05 = 5/100 = frac(1, 2^2 times 5) $
となるため$2$進法では表せない。また$0.875$は
$ 0.875 = 875/1000 = 7/2^3 (= frac(2^2 + 2^1 + 2^0, 2^3)) $
となるため$0.875 = 0.111_((2))$と表される。

これより正しい選択肢は2.である。
#answer(2)

これは*丸め誤差*に深く関連している。多くの計算機で
$ 0.1 + 0.2 = 0.30000000000000004 $
などと計算されてしまうのは、$0.1$も$0.2$も$2$進法では有限小数で表せないためである。

補足として、$N$進法において有理数$q$が有限小数で表されるための必要十分条件は、互いに素な整数$m$、$n$（ただし$n > 0$）が存在して
$ q = frac(m, n) $
とかけて（既約分数）かつ$n$の素因数が$N$の素因数のみで構成されることである。

例えば既約分数の分母に$2$と$5$以外の素因数を含む有理数は、$10$進法では循環小数となる。
